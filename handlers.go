...
func base(title, nav string) map[string]any {
...
func handleIndex(w http.ResponseWriter, r *http.Request) {
	...
	// draft alag dikhane hain - author ko pata hona chahiye kya publish nahi hua
	var live, drafts []*Post
	for _, p := range posts {
		if p.Published {
			live = append(live, p)
		} else {
...
func handlePost(w http.ResponseWriter, r *http.Request) {
	...
	// view count badha do. Alag goroutine me isliye kyunki isme file write hoti hai
...
func handleEditor(w http.ResponseWriter, r *http.Request) {
	...
	d := base(map[bool]string{true: "Naya post", false: "Edit: " + p.Title}[isNew], "new")
	d["Post"] = p
	d["IsNew"] = isNew
...
func handleGraphPage(w http.ResponseWriter, r *http.Request) {
	render(w, "graph", base("Blog ka naksha", "graph"))
}
...
func apiRestore(w http.ResponseWriter, r *http.Request) {
	...
	if err := store.RestoreRevision(in.Slug, in.File); err != nil {
